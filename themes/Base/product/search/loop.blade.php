...
?>
<div class="bc-product cart{{ $class ?? "" }}">
    <div class="bc-product__thumbnail">
        <a href="{{$row->getDetailUrl()}}" class="ratio ratio-4x3 d-block">
        {!! get_image_tag($row->image_id,'medium',['alt'=>$translation->title,'class'=>'object-cover']) !!}
        </a>
        @if($row->stock_status == "in")
            @if(!empty($row->discount_percent))
                <div class="bc-product__badge">-{{$row->discount_percent}}</div>
            @endif
        @else
            <span class="bc-product__badge out-stock">{{__('Out Of Stock')}}</span>
        @endif
    ...
    <div class="bc-product__container">
        @if(is_vendor_enable() and $row->author)
            <a class="bc-product__vendor" href="{{$row->author->getDetailUrl()}}">{{$row->author->display_name}}</a>
        @endif
        <div class="bc-product__container">
            <a class="bc-product__title" href="{{$row->getDetailUrl()}}">{{$translation->title}}</a>
            @if(!empty($reviewData['total_review']))
            <div class="bc-product__rating mb-2 d-flex mt-1">
                @include('global.rating',['percent'=>$score_total * 2 * 10 ?? 0])
                <span>{{$reviewData['total_review']}}</span>
            </div>
            @endif
            <p class="bc-product__price sale">{{$row->display_price}}
