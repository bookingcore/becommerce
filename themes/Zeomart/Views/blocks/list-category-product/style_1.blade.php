                ...
                <div class="flex" data-tabs-toggle="#zm-tab-toggle-{{ $id }}">
                    @if(!empty($categories))
                        @foreach($categories as $item)
                            <button data-tabs-target="#zm-tab-{{ $item->id }}" aria-controls="zm-tab-{{ $item->id }}" role="tab" aria-selected="false" class="font-[500] text-base relative pb-1 ml-4 ">
                                {{ $item->translate()->name }}
                            </button>
                        @endforeach
                    @endif
                    <button data-tabs-target="#zm-tab-all" aria-controls="zm-tab-all" role="tab" aria-selected="false" class="font-[500] text-base relative pb-1 ml-4">
                        {{ __("All") }}
                    </button>
                </div>
            </div>
            <div class="bc-content" id="zm-tab-toggle-{{ $id }}">
                <div class="hidden" id="zm-tab-all"  role="tabpanel">
                ...
                @foreach($categories as $item)
                    <div class="hidden" id="zm-tab-{{ $item->id }}"  role="tabpanel">
