...
<div class="bc-loop-product relative h-full {{$class ?? ""}} group overflow-hidden">
    <div class="mb-5">
        <a href="{{$row->getDetailUrl()}}" class="mi-h-230 relative overflow-hidden block">
            ...
            <button type="button" class="translate-y-full group-hover:translate-y-0 opacity-0 actions absolute bottom-0 w-full group-hover:opacity-100  duration-300 focus:outline-none bg-[#F5C34B] hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-3.5">
                {{__('View Detail')}}
            </button>
        </a>
        <div class="opacity-0 actions absolute top-0 right-0 group-hover:opacity-100  duration-300 translate-x-full group-hover:translate-x-0">
            <div class="service-wishlist is_loop {{$row->isWishList()}} w-9 h-9 border flex items-center justify-center rounded-full cursor-pointer bg-white hover:bg-[#F5C34B]  duration-300" data-id="{{$row->id}}" data-type="{{$row->type}}" data-bs-toggle="tooltip"  title="{{ __("Wishlist") }}">
            ...
            <div class="bc-compare mt-1 w-9 h-9 border flex items-center justify-center rounded-full cursor-pointer bg-white hover:bg-[#F5C34B]  duration-300" data-bs-toggle="tooltip" title="{{ __("Compare") }}" data-id="{{$row->id}}">
