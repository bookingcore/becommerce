...
class MenuWalker
{
    ...
    public function generate($options = [])
    {
        ...
        if (!empty($items)) {
            ...
            echo "<ul class='flex flex-col font-medium md:flex-row space-x-5 text-base -ml-2 {$options['class']}'>";
    ...
    public function generateTree($items = [], $depth = 0, $parentKey = '')
    {
        ...
        foreach ($items as $k => $item) {
            ...
            if (!empty($item['children'])) {
                ...
                $toggle = 'data-dropdown-toggle="mega-menu-dropdown-' . $k . '"';
            ...
            if (!empty($item['children'])) {
                echo '<div id="mega-menu-dropdown-' . $k . '" class="absolute z-10 hidden min-w-[200px] p-5 text-sm bg-white border border-gray-100 rounded-b shadow-md">';
