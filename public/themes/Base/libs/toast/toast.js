window.BCToast = {
    ...
    showAjaxSuccess:function(data,timeout){
        console.log(data);
        if(data.message){
    ...
    _show:function (icon,content,timeout,type){
        ...
        var temp = `<div class="d-flex p-1">
            <div class="toast-body d-flex align-content-center">
                <div class="toast-icon me-1">${icon}</div>
                ${content}
           </div>
        ...
        el.className = 'bc-toast toast align-items-center position-relative';
        el.innerHTML = temp;
