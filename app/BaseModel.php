...
class BaseModel extends Model
{
    ...
    public function translateOrOrigin($locale = false){
        if(empty($locale)) $locale = setting_item('site_locale');
...
//        }
    ...
    public function saveOriginOrTranslation($locale = false,$saveSeo = true)
    {
        ...
        $site_enable_multi_lang = setting_item('site_enable_multi_lang');
        if(!$locale or is_default_lang($locale) or empty($site_enable_multi_lang)){
            $res = $this->save();
            if($res && $saveSeo){
    ...
    public static function findWithCache($id){
        return Cache::rememberForever(static::getCacheKey($id),function() use ($id){
    ...
    protected static function getCacheKey($id){
        ...
        $a = new self();
        return $a->getTable().'_'.$id;
    }

    public function clearCache(){
        if(!$this->id) return;
        if(static::getCacheKey($this->id)){
