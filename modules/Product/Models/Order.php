...
class Order extends BaseModel{
    ...
    public function getMeta($key, $default = '')
    {
        ...
        $val = DB::table('product_order_meta')->where([
            'order_id' => $this->id,
            'meta_key'       => $key
        ])->first();
        if (!empty($val)) {
    ...
    public function addMeta($key, $val, $multiple = false)
    {
        ...
        if (is_object($val) or is_array($val))
            $val = json_encode($val);
        if ($multiple) {
            return DB::table('product_order_meta')->insert([
                'meta_key'       => $key,
                'meta_value'        => $val,
                'order_id' => $this->id
            ]);
        } else {
            $old = DB::table('product_order_meta')->where([
                'order_id' => $this->id,
                'meta_key'       => $key
            ])->first();
            if ($old) {

                return DB::table('product_order_meta')->where('id', $old->id)->insert([
                    'meta_value' => $val
                ]);
    ...
    public static function getTopCardsReport()
    {
        ...
        $res = [];
        $total_data = parent::selectRaw('sum(`total`) as total_price , sum( `total` - `total_before_fees` ) AS total_earning ')->whereNotIn('status',static::$notAcceptedStatus)->first();
    ...
    public static function getDashboardChartData($from, $to)
    {
        ...
        $sql_raw[] = 'sum( `total` - `total_before_fees`  ) AS total_earning';
    ...
    public function getCommissionVendor($vendorId,$total){
        $returnArray=[
            'commission'=>null,
        ...
        if (setting_item('vendor_enable') == 1) {
            ...
            if (!empty($vendor)) {
                ...
                $commission['type'] = setting_item('vendor_commission_percent', 'percent');
                ...
                if ($commission['type'] == 'percent') {
                    $returnArray['commission'] = ($total / 100) * $commission['amount'];
                } else {
                    $returnArray['commission']= $commission['amount'];
                }
