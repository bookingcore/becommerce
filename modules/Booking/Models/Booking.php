...
class Booking extends BaseModel
{
    ...
    {
        $val = DB::table('core_booking_meta')->where([
            'booking_id' => $this->id,
            'name'       => $key
        ])->first();
        if (!empty($val)) {
    ...
    public function addMeta($key, $val, $multiple = false)
    {
        ...
        if (is_object($val) or is_array($val))
            $val = json_encode($val);
        if ($multiple) {
            return DB::table('core_booking_meta')->insert([
                'name'       => $key,
                'val'        => $val,
                'booking_id' => $this->id
            ]);
        } else {
            $old = DB::table('core_booking_meta')->where([
                'booking_id' => $this->id,
                'name'       => $key
            ])->first();
            if ($old) {

                return DB::table('core_booking_meta')->where('id', $old->id)->insert([
                    'val' => $val
                ]);
